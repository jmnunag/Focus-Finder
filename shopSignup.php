...
<!-- First Form in shopsignup.php -->
<div class="form-section" id="personal-info-form">
    <div class="suggestion-form-box">
        <div class="signup-text">Owner Sign Up</div>
        <!-- Add form tag here -->
        ...
        <label>Email</label>
            ...
            <label class="personal-info-field">Full Name</label>
            <div class="input-group personal-info-field">
                <input type="text" id="fullname" name="fullname" placeholder="Enter your name" required>
            </div>
                                
            <label class="personal-info-field">Password</label>
            <div class="input-group personal-info-field">
...
• At least one special character (!@#$%^&*(),.?':{}|<>)"></i>
            ...
            <label class="personal-info-field">Confirm Password</label>
            <div class="input-group personal-info-field">
                <div class="password-container">
        ...
        <a href="signup.php">Not a Shop Owner? Register as a User</a>
    </div>
</div>

<!-- Second Form -->
<div class="form-section" id="shop-info-form" style="display: none;">
    <div class="suggestion-form-box">
        <div class="signup-text">Establishment Registration</div>
        <form action="process_shop_signup.php" method="post" enctype="multipart/form-data" onsubmit="return validateForm()">
            <!-- Add hidden fields for personal info -->
...
function showNextForm() {
    ...
    if (!email || !emailRegex.test(email)) {
        document.querySelector('input[type="email"]').closest('.input-group').classList.add('error-border');
        errorMessage.textContent = 'Please enter a valid email address';
        errorMessage.style.display = 'block';
        return;
    }

    fetch('check_email_status.php', {
        method: 'POST',
        headers: {
    ...
    .then(data => {
        if (!data.available && !data.userData) {
            document.querySelector('input[type="email"]').closest('.input-group').classList.add('error-border');
        ...
        document.getElementById('hidden_fullname').value = data.userData.fullname;
        document.getElementById('hidden_email').value = email;
        document.getElementById('hidden_password').value = data.userData.password;
        
        if (data.userData.contact) {
            document.getElementById('Cnumber').value = data.userData.contact;
        }
            
        proceedToNextForm();
        } else {
            personalInfoFields.forEach(field => field.style.display = 'block');
            ...
            if (!passwordRegex.test(password)) {
                document.querySelector('#password').closest('.input-group').classList.add('error-border');
                errorMessage.textContent = 'Password must contain 8-16 characters, at least one uppercase letter, one lowercase letter, one number, and one special character';
                errorMessage.style.display = 'block';
                return;
            }
            ...
            if (password !== cpassword) {
                document.querySelector('#password').closest('.input-group').classList.add('error-border');
                document.querySelector('#cpassword').closest('.input-group').classList.add('error-border');
...
function proceedToNextForm() {
    ...
    document.getElementById('shop-info-form').style.display = 'block';
...
function showPreviousForm() {
    document.getElementById('shop-info-form').style.display = 'none';
